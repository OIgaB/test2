...
const Map1 = () => {
  ...
  const fetchData = async backendData => {
    try {
      const data = await Promise.all(
        backendData.map(async ({ country, teachersPercentage }) => {
      ...
      const descendingCountryOrder = [...data].sort(
  ...
  useEffect(() => {
    ...
    const uniqueTeachersCountries = teachersCountries.filter(
    ...
    const backendData = uniqueTeachersCountries.map(uniqueCountry => {
      ...
      const countryClones = teachersCountries.filter(
        country => country === uniqueCountry
      );
      // ['Ukraine', 'Ukraine', 'Ukraine']
      // ['Poland', 'Poland']
      // ['Germany'] ...
      const teachersPercentagePerCountry = Math.round(
        (countryClones.length * 100) / teachersNumber
      );
  ...
  useEffect(() => {
    // поява маркерів тільки, коли докрутили мишкою до карти
